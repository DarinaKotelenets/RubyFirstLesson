...
module MY_METHODS
  ...
  def register(user_login, user_pass, user_firstname, user_lastname, user_mail, lang)
    ...
    @driver.find_element(:css, "#new_user>input[type='submit']").click
  end
  ...
  def log_in(_pass, _login)
    ...
    @driver.find_element(:css, "#login-form>input[type = 'submit']").click
  end

  def change_pass (pass, newpass)
    @driver.find_element(:class, 'my-account').click
    @driver.find_element(:css, "#icontent>a[href$ = 'password']").click
    ...
    @driver.find_element(:css, "#content>input[type='submit']").click
  end
  ...
  def create_new_project(project_name, project_id)
    ...
    @driver.find_element(:xpath, "#new_project>input[name='commit']").click
  end
  ...
  def add_new_member
    ...
    @wait.until {
      !@driver.find_elements(:css, "#new_membership'>input[value='5']").empty? }
    role = @driver.find_element(:css, "#new_membership'>input[value='5']")
    if role.attribute('checked'=='true')
      @driver.find_element(:css, "#new_membership'>input[value='5']").click
    end
